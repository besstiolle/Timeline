<script lang="ts">
	...
	function toStringDate(date: Date): string {
		...
		return date.getDate().toString().padStart(2, '0') 
			...
			+ DATE_SEPARATOR + (date.getFullYear()).toString().padStart(2, '0') 
	...
	<div class="basis-1/3">
		<div class="max-w-95/100 m-auto mt-5 flex shadow-xl/30 bg-blue-100 dark:bg-slate-800 cursor-pointer" 
			...
			<div class="flex-2/3 p-2">
				<div class="relative">
					...
					<div id="menu-toggle-{index}" class='float-right relative' 
						...
						<div id="menu-{index}" class="menus hidden absolute -top-5 w-50 z-10 bg-blue-100 dark:bg-slate-800 shadow-xl/30">
							...
							<!-- Button delete-->
							<div class:hidden={card.isOnline} class="p-2
							...
							<div class:hidden={!card.isOnline} class="p-2
										border-t-1 border-blue-300 dark:border-slate-900 
										">
								<svg viewBox="0 0 600 600" class='float-left size-6 fill-gray-800 dark:fill-blue-50'><use x="5" y="75" href="#ico_cloud"/></svg>
				...
				<p class="text-xs">{m.landing_updated_text()} : {#if card.lastUpdated}{toStringDate(card.lastUpdated)}{/if}</p>
				<div class=''>
