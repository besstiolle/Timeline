...
<header
	class="bg-blue-50 dark:bg-gray-800 shadow-2xl border-b border-sky-700 h-12 flex items-center justify-between px-4"
>
	...
	<div class="flex items-center space-x-4">
			...
			{#if isDark}
				...
				<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 32 32">
					<desc /><defs /><g
						...
						stroke="none"
						stroke-width="1"
						><g class="stroke-blue-50" stroke-width="2.5" transform="translate(2.000000, 2.000000)"
							><circle cx="14" cy="14" id="Oval-4" r="8" /><path
		...
		<langs>
			{#each locales as locale, index (index)}
				...
				{:else}
					<button type="button" onclick={() => setLocale(locale)} class="cursor-pointer"
						>{locale}</button
					>{#if locales_ln !== index}|{/if}
				{/if}
			{/each}
		</langs>
	</div>
