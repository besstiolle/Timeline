...
<ShadowBox bind:this={shadowBox} id="droppable">
	...
	<form method="post" action="" enctype="multipart/form-data" use:svelteAction>
		<div>
			...
			<label for="file"
				><span
					class="w-50 m-auto mt-2 flex gap-2 rounded-full shadow-xl/15 bg-linear-to-r/srgb from-cyan-600 to-emerald-500 hover:bg-linear-to-r/hsl hover:to-cyan-600 hover:from-emerald-500 p-3 cursor-pointer"
					>{m.upload_label_action()}</span
				>
				{m.upload_label_action_text()}</label
			>
		</div>
		<!--<button type="submit">{m.upload_action()}</button>-->
		<div>
			<span
				class="w-40 m-auto mt-2 flex gap-2 rounded-full shadow-xl/15 bg-linear-to-r/srgb from-cyan-600 to-emerald-500 hover:bg-linear-to-r/hsl hover:to-cyan-600 hover:from-emerald-500 p-3 cursor-pointer"
				onclick={downloadCsv}
				onkeydown={downloadCsv}
				role="button"
				tabindex="0">{m.upload_label_download_csv_action()}</span
			>
			{m.upload_label_download_csv_action_text()}
		</div>
		<div>
			<span
				class="w-40 m-auto mt-2 flex gap-2 rounded-full shadow-xl/15 bg-linear-to-r/srgb from-cyan-600 to-emerald-500 hover:bg-linear-to-r/hsl hover:to-cyan-600 hover:from-emerald-500 p-3 cursor-pointer"
				onclick={downloadToml}
				onkeydown={downloadToml}
				role="button"
				tabindex="0">{m.upload_label_download_toml_action()}</span
			>
