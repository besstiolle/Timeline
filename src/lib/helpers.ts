...
export class Helpers {
	 ...
	 */
	static toYYYYMMDD_hhmm(date: Date): string {
		return (
	 ...
	 */
	static getDateFromViewportX(x: number, dmin: Date, dmax: Date): Date {
		if (x < GRID.MIDDLE_X) {
			x = GRID.MIDDLE_X;
		}
		...
		const date = new Date(
			((Math.ceil(x) - GRID.MIDDLE_X) * (dmax.getTime() - dmin.getTime())) / GRID.MIDDLE_WIDTH +
				dmin.getTime()
		);
	 ...
	 **/
	static getViewportXFromDate(date: Date, dmin: Date, dmax: Date): number {
		const x =
			((date.getTime() - dmin.getTime()) * GRID.MIDDLE_WIDTH) / (dmax.getTime() - dmin.getTime()) +
			GRID.MIDDLE_X;
	 ...
	 * @returns the id
	 */
	static randomeString(length: number) {
		let result: string = '';
	 ...
	 */
	static printf(str: string, args: (string | number)[]) {
	 ...
	 */
	static getEstimationOfDiff(dateMin: Date, dateMax: Date): string {
		const diff = dateMax.getTime() - dateMin.getTime();
		if (diff > TIMERS.YEARS20) {
			return DIFF.isMoreThan20Years;
		}
		if (diff <= TIMERS.YEARS20 && diff > TIMERS.YEARS10) {
			return DIFF.isBetween10YearsAnd20Years;
		}
		if (diff <= TIMERS.YEARS10 && diff > TIMERS.YEARS6) {
			return DIFF.isBetween6YearsAnd10Years;
		}
		if (diff <= TIMERS.YEARS6 && diff > TIMERS.YEARS3) {
			return DIFF.isBetween3YearsAnd6Years;
		}
		if (diff <= TIMERS.YEARS3 && diff > TIMERS.MONTHS20) {
			return DIFF.isBetween20MonthsAnd3Years;
		}
		if (diff <= TIMERS.MONTHS20 && diff > TIMERS.MONTHS5) {
			return DIFF.isBetween5MonthsAnd20Months;
		}
		if (diff <= TIMERS.MONTHS5 && diff > TIMERS.DAYS31) {
			return DIFF.isBetween1MonthAnd5Months;
		}
		if (diff <= TIMERS.DAYS31) {
			return DIFF.isBelow1Month;
		}
