...
<svg viewBox="{$store.currentTimeline.viewbox}" xmlns="http://www.w3.org/2000/svg" 
    ...
    role="none">
        ...
        {#if hasSwimline}
        <text text-anchor="end" x="{xGrayPosition - 5}" y="10.5" font-size="9" class="primaryFill">{currentTask.label}</text>
        {:else}
        <text text-anchor="end" x="{GRID.MIDDLE_X - 5}" y="10.5" font-size="9" class="primaryFill">{currentTask.label}</text>
        <line stroke-dasharray="0.5 2" x1="{GRID.MIDDLE_X}" y1="8" x2="{xGrayPosition - 5}" y2="8" class="secondaryStroke" />
        {/if}
        ...
        <text id="T{currentTask.id}_rlabel" x="{x2GrayPosition + 5}" y="10.5" class="secondaryFill">{labelRight}</text>
        <!-- Draggable overlay -->
