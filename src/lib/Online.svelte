...
import Toast from "./Toast.svelte";
    ...
    function doOffline(){
        ...
        remove($store.currentTimeline.key, $store.currentTimeline.ownerKey).then((json) => {
            ...
            //Rewrite URL
            window.location.href= $page.url.protocol + '//' + $page.url.host + "/g/" + $store.currentTimeline.key
...
<form id="box" class:hidden method="post" action="" enctype="multipart/form-data">
    <div>
        {#if $store.currentTimeline.isOnline}
            ...
            <div><label for='readOnly'>Read-only URL : </label><input id='readOnly' readonly type='text' value='{$page.url.protocol + '//' + $page.url.host + "/g/" + $store.currentTimeline.key + "?r=" + $store.currentTimeline.readKey}'></div>
            <div><label for='writer'>Writer URL : </label><input id='writer' readonly type='text' value='{$page.url.protocol + '//' + $page.url.host + "/g/" + $store.currentTimeline.key + "?w=" + $store.currentTimeline.writeKey}'></div>
            <div><label for='owner'>Owner URL : </label><input id='owner' readonly type='text' value='{$page.url.protocol + '//' + $page.url.host + "/g/" + $store.currentTimeline.key + "?o=" + $store.currentTimeline.ownerKey}'></div>
        {:else}
